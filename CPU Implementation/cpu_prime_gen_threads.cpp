...
int main()
{
	...
	/**
	 * Calculate the amount of numbers that each thread will test for primality.
	 */
	per_thread = (rn - ln)/thread_n + 1;
	ulli nln = ln, nrn =ln;

	/**
	 * Run a loop that will create threads where each thread has same amount of numbers
	 * and push the thread into the vector
	 */
	for(int i = 1; i <= thread_n; ++i)
	{
		nln = nrn;
		nrn = (nln + per_thread) < rn ? (nln + per_thread) : rn; 
